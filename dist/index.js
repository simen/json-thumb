...
function shapesEqual(a, b) {
    ...
    switch (a.kind) {
        ...
        case "array":
            return shapesEqual(a.children, b.children);
        case "object": {
            const bb = b;
            ...
            for (let i = 0; i < aKeys.length; i++) {
                if (aKeys[i] !== bKeys[i])
                    return false;
                if (!shapesEqual(a.keys[aKeys[i]].shape, bb.keys[bKeys[i]].shape))
                    return false;
            }
            return true;
        }
        case "varied": {
            const bb = b;
            if (a.variants.length !== bb.variants.length)
                return false;
...
function mergeShapes(a, b) {
    ...
    if (a.kind === b.kind) {
        switch (a.kind) {
            ...
            case "object":
                return mergeObjectShapes(a, b);
            case "array":
                return mergeArrayShapes(a, b);
        }
    }
    // Different kinds → varied
    return { kind: "varied", variants: [a, b] };
}
function mergeObjectShapes(a, b) {
 ...
 */
function addToVariants(variants, shape) {
 ...
 */
function sampleIndices(length, sampleSize) {
 ...
 * @returns Shape AST
 */
export function inferShape(value, options) {
    const sampleSize = options?.sampleSize ?? DEFAULT_SAMPLE_SIZE;
    const maxDepth = options?.maxDepth ?? DEFAULT_MAX_DEPTH;
    return _inferShape(value, sampleSize, maxDepth, 0);
}
function _inferShape(value, sampleSize, maxDepth, depth) {
    ...
    if (t === "object") {
        return _inferObjectShape(value, sampleSize, maxDepth, depth);
    }
    // Fallback for exotic types (bigint, symbol, function, etc.)
    return { kind: "scalar", type: "string" };
}
function _inferArrayShape(arr, sampleSize, maxDepth, depth) {
...
function _inferObjectShape(obj, sampleSize, maxDepth, depth) {
 ...
 */
function isCompactObject(shape) {
 ...
 */
function formatArrayLength(shape) {
 ...
 */
function renderShape(shape, indent, maxDepth, depth) {
    switch (shape.kind) {
        case "scalar":
            return shape.type;
        case "varied":
...
function renderArray(shape, indent, maxDepth, depth) {
...
function renderObject(shape, indent, maxDepth, depth) {
    const keys = Object.keys(shape.keys);
 ...
 * ```
 */
export function thumb(value, options) {
    const maxDepth = options?.maxDepth ?? DEFAULT_MAX_DEPTH;
